...
def train_model(model, train_loader, val_loader, device, num_epochs, lr, model_save_path, df):
    ...
    criterion_class = nn.CrossEntropyLoss(weight=class_weights_tensor, label_smoothing=0.1)
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels, weights in train_loader:
            ...
            loss = 0.95 * criterion_class(outputs_class, labels) + 0.05 * criterion_weight(outputs_weight, weights)
            loss.backward()
            optimizer.step()
        ...
        if val_loss < best_val_loss:
            ...
            print(f"\n✅ Saved best model at epoch {epoch + 1} with Val Loss={val_loss:.4f}, Acc={acc:.4f}, MAE={mae:.2f}")
...
if __name__ == '__main__':
    ...
    print(classification_report(
      ...
      zero_division=0  # avoids warning when some labels are missing
    ))


    # Scatter Plot for Regression
    plt.figure(figsize=(8, 6))
    plt.scatter(all_weight_true, all_weight_preds, alpha=0.6, edgecolor='k')
    plt.plot([min(all_weight_true), max(all_weight_true)], [min(all_weight_true), max(all_weight_true)], 'r--')
    plt.xlabel("True Weight (g)")
    plt.ylabel("Predicted Weight (g)")
    plt.title("Weight Prediction: True vs Predicted")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
