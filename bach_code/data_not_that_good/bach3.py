...
def train_model(model, train_loader, val_loader, device, num_epochs, lr, model_save_path, train_df):
    ...
    local_model_path = os.path.join(model_save_path, "best_model.pth")
    external_model_path = "/kaggle/input/best-model-local/best_model.pth"
    load_path = local_model_path if os.path.exists(local_model_path) else external_model_path if os.path.exists(external_model_path) else None
